...
#else
unsigned int w_pistiong1,w_pistiong2,fatul;
//靠背电动缸位置 最高：0 最低：最大位置
...
//返回靠背电动缸的绝对位置，靠时间来记录，靠背电动缸位于最高处时间为0，单位10ms为
unsigned int BackMotor_Get_Position(void)
{
  return w_Position;
}
	...
	else
	{
		...
		switch(nFinalBackPadMotorState)
		{
			case STATE_RUN_BACK_DOWN:  //back motor go down
				...
				bPowerFlag = TRUE;
     /* if(nReworkShoulderPosition == 1)
      {
        nReworkShoulderPosition = 2;//zlw
        bGetNextActionStep = TRUE;
        //nCurActionStep = 0;//zlw
      }*/
				break ;
			case STATE_RUN_BACK_UP:  //back motor go up
				if(Input_GetBackUpSwitch() == REACH_BACK_LIMIT || position == 2)
