...
#define ALIGNMENT1(base,align) (((base)+((align)-1))&(~((align)-1)))

/* Packet storage. Double buffered version. */
#if defined (__ICCARM__)
#pragma data_alignment=4
uint8_t rawPacketl[2][ALIGNMENT1(sizeof(XMODEM_packet),4)];
#elif defined (__CC_ARM)
uint8_t rawPackelt[2][ALIGNMENT1(sizeof(XMODEM_packet),4)] __attribute__ ((aligned(4)));
#elif defined (__GNUC__)
uint8_t rawPacketl[2][ALIGNMENT1(sizeof(XMODEM_packet),4)] __attribute__ ((aligned(4)));
#else
 ...
 *****************************************************************************/
__ramfunc __INLINE int XMODEMLEUART_verifyPacketChecksum(XMODEM_packet *pkt, int sequenceNumber)
{
  uint16_t packetCRC;
 ...
 *****************************************************************************/
__ramfunc int XMODEMLEUART_download(uint32_t baseAddress, uint32_t endAddress)
{
  XMODEM_packet *pkt;
  uint32_t      i;
  uint32_t      addr;
...
#endif  */
        if (BOOTLOADER_LEUART->STATUS & LEUART_STATUS_RXDATAV)//2017  
 ...
 xmodem_transfer:
  while (1)
  {
    ...
    {
      *(((uint8_t *) pkt) + byte) = LEUART_rxByte();
    }

    if (XMODEMLEUART_verifyPacketChecksum(pkt, sequenceNumber) != 0)
    {
      /* On a malformed packet, we send a NAK, and start over */
      LEUART_txByte(XMODEM_NAK);
      continue;
    }
